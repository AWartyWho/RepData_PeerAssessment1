...
The mean total steps taken per day is **`r format(steps_mean, scientific=FALSE)`**, and the median total steps taken per day is **`r steps_median`**.
...
The interval that contains the maximum number of steps is **`r max_interval` to `r (max_interval + 5)` minutes**.
...
The mean total steps taken per day is **`r format(imputed_mean, scientific=FALSE)`**, and the median total steps taken per day is **`r format(imputed_median, scientific=FALSE)`**. As you can see, the mean is identical to the dataset without imputation, while the median is slightly different and equal to the mean (due to imputation around the 'centre'). Overall, it is clear that the overall estimates are only slightly changed.
